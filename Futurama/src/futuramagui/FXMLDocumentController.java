...
import futurama.iGame;
import futurama.Game;
import java.net.URL;
...
public class FXMLDocumentController implements Initializable {
    ...
    @FXML
    private ChoiceBox<ListHolderInformation> scenariosCB; //Holds possible scenarios
    @FXML
    ...
    private ObservableList<ListHolderInformation> npcChoices = FXCollections.observableArrayList();
    private ObservableList<ListHolderInformation> scenarios = FXCollections.observableArrayList();
    ...
    /**
     * Sets the scene as the solar system. This includes creating and placing
     * buttons for each planet and adding style and functionality to these.
     */
    @FXML
    ...
    /**
     * Clears all nodes from the scene gridpane. This is called before every new
     * scene is loaded.
     */
    ...
    /**
     * Sets the scene of the new planet and moves player to it. Also updates the
     * NPC choicebox and loads selected NPC to the scene.
     *
    ...
    public void planetHandle(UUID planetUuid) {
        ...
        if (this.game.getPossiblePlanets().contains(planetUuid) || warpRB.isSelected()) {
            ...
            for (UUID npc : this.game.getAvailableNpcs(planetUuid)) {
                this.npcChoices.add(new ListHolderInformation(npc, this.game.getName(npc)));
            }
            ...
            this.npcCB.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    ...
                    Image npcImage = new Image(game.getImgPath((((ListHolderInformation) npcCB.getValue()).getUuid())));
                    npcButton.setGraphic(new ImageView(npcImage));
                    npcButton.setOnAction(new EventHandler<ActionEvent>() {
                        @Override
                        public void handle(ActionEvent event) {
                            game.startConversation(((ListHolderInformation) npcCB.getValue()).getUuid());
    ...
    public void handleStart(ActionEvent event) {
        ...
        if (this.scenariosCB.getValue() == null) {
            ...
            this.game.startGame(scenariosCB.getValue().getUuid(), this.nameTF.getText());
            this.realTimeTimer();
            this.updateStats();
    ...
    /**
     * Updates the dialogue text area and answer buttons while a conversation is
     * ongoing. When conversation ends, the NPC button (and image) gets removed
     * from screen, and the buttons let player see the solar system or travel to
    ...
    /**
     * Updates each inventory slot only if necessary by checking the size of
     * player's inventory.
     */
    ...
    /**
     * Checks which button is pressed and drops the corresponding package from
     * player's inventory.
     *
    ...
    public void realTimeTimer() {
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                timeTA.setText("" + (playedTime.get(Calendar.HOUR) - 1) + ":"
    ...
    /**
     * Loads the help text into a hashmap used for displaying the text on the
     * help screen.
     */
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        for (UUID scenario : this.game.getPossibleScenarios()) {
            this.scenarios.add(new ListHolderInformation(scenario, this.game.getName(scenario)));
        }
